...
class Summary {
  ...
  constructor(itemData) {
    ...
    /* istanbul ignore next */ this.tradingTax = itemData.trading_tax || 0;
  ...
  toString(opt = 'item') {
    let value = this.name;
    /* istanbul ignore if */ if (opt === 'codex' || opt === 'all') {
      value += `\`\`\`\nCodex: ${this.codex}\n`;
    }
    /* istanbul ignore else */ if (opt === 'item' || /* istanbul ignore next */ opt === 'all') {
    ...
    /* istanbul ignore next */ if (opt === 'location' || opt === 'all') {
