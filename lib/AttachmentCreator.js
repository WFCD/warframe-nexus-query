...
function fieldValue(nComponent, mComponent, mCount) {
  ...
  if (nComponent) {
    // console.log(`Building nexus component.... ${nComponent.name} : ${nComponent.prices.median}`);
  ...
  if (nComponent && mComponent) {
    return {
      name: `${mCount.length < 2 ? '_ _' : mComponent.name.replace(nComponent.title, '').trim()}${nComponent.ducats ? ` - ${nComponent.ducats} ducats` : ''}`,
      value: '```haskell\n'
    ...
    return {
      name: `${nComponent.name.trim()}${nComponent.ducats ? ` - ${nComponent.ducats} ducats` : ''}`,
      value: `${'```haskell\n'
