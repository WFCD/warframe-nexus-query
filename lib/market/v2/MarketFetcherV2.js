...
export default class MarketFetcherV2 {
  ...
  async getTopOrders(slug, options = {}) {
    ...
    const cacheKey = `top_orders_${slug}_${platform}_${JSON.stringify(filters)}`;

    // Check short-lived cache
    const cached = this._getOrdersFromCache(cacheKey);
    ...
    this._setOrdersCache(cacheKey, result);
    return result;
  }
