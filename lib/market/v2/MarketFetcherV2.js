...
import { extractOrderFilters, labelSummaryForFilters } from '../../priceCheckOptions.js';
...
export default class MarketFetcherV2 {
  ...
  async queryMarket(query, options = {}) {
    const { platform, successfulQuery, ranks: _ranks, ...rest } = options;

    if (!platform) {
      throw new Error('Platform is required');
    }

    const orderFilters = extractOrderFilters(rest);
    ...
    const topOrders = await this.getTopOrders(item.slug, { platform, ...orderFilters });
    ...
    const summary = labelSummaryForFilters(
      new Summary(item, topOrders, {
        sell: sellStats,
        buy: buyStats,
      }),
      orderFilters
   ...
   * @param {Object} [options={}] - Optional order filters (rank, ranks, rankLt, etc.)
   * @returns {Promise<Object>}
   */
  async priceCheckQuery(query, platform, options = {}) {
    return this.queryMarket(query, { platform, ...options });
  }
